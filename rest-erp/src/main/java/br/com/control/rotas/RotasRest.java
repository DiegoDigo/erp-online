package br.com.control.rotas;

public class RotasRest {

	public static final String RAIZ = "/servicos/erp";
	public static final String RAIZ_PRODUTO = "/produto";
	public static final String RAIZ_CLIENTE = "/cliente";
	public static final String RAIZ_TIPO_ENDERECO = "/tipoEndereco";
	public static final String RAIZ_FAMILIA = "/familia";
	public static final String RAIZ_GRUPO = "/grupo";
	public static final String RAIZ_MARCA = "/marca";
	public static final String RAIZ_CATEGORIA = "/categoria";
	public static final String RAIZ_DETALHE_COMBO = "/detalhe/combo";
	public static final String RAIZ_CANAL = "/canal";
	public static final String RAIZ_ACOMPANHAMENTO = "/acompanhamento";
	public static final String RAIZ_CADASTRO = "/cadastro";
	public static final String RAIZ_TIPO = "/tipo";
	public static final String RAIZ_TIPO_COBRANCA_CLIENTE = "/tipoCobrancaCliente";
	public static final String RAIZ_COBRANCA = "/cobranca";
	public static final String RAIZ_ENDERECO = "/endereco";
	public static final String RAIZ_OCORRENCIA = "/ocorrencia";
	public static final String RAIZ_CONDICAO_PAGAMENTO = "/condicao";
	public static final String RAIZ_VENDEDOR = "/vendedor";
	public static final String RAIZ_VENDEDOR_CLIENTE = "/vendedorCliente";
	public static final String RAIZ_PRECO = "/preco";
	public static final String RAIZ_BANDA = "/banda";
	public static final String RAIZ_BANDA_ITEM = "/bandaItem";
	public static final String RAIZ_PRODUTO_CANAL = "/produtoCanal";
	public static final String RAIZ_IMAGEM = "/imagem";
	public static final String RAIZ_PEDIDO = "/pedido";
	public static final String RAIZ_PEDIDO_SUGESTAO = "/pedidoSugestao";
	public static final String RAIZ_CAPA = "/capa";
	public static final String RAIZ_ITEM = "/item";
	public static final String RAIZ_HISTORICO = "/historico";
	public static final String RAIZ_HISTORICO_ITEM = "/historicoItem";
	public static final String RAIZ_CHAVE_NFE = "/chaveNfe";
	public static final String RAIZ_ORIGEM_LOGRADOURO = "/origem/logradouro";
	public static final String RAIZ_MOTIVO = "/motivo";
	public static final String RAIZ_GERAL = "/geral";
	public static final String RAIZ_MOVIMENTO = "/movimento";
	public static final String RAIZ_PEDIDO_BLOQUEADO = "/pedidoBloqueado";
	public static final String RAIZ_FINANCEIRO = "/financeiro";
	public static final String RAIZ_COMODATO = "/comodato";
	public static final String RAIZ_RESTRICAO = "/restricao";
	public static final String RAIZ_RESTRICAO_ITEM = "/restricaoItem";
	public static final String RAIZ_FINANCEIRA = "/financeira";
	public static final String RAIZ_COMERCIAL = "/comercial";
	public static final String RAIZ_LIBERADO = "/liberado";
	public static final String RAIZ_RECUSADO = "/recusado";
	public static final String RAIZ_BLOQUEADO = "/bloqueado";
	public static final String RAIZ_SUGESTAO = "/sugestao";
	public static final String RAIZ_PAROCO = "/paroco";
	public static final String RAIZ_CORTADO = "/cortado";
	public static final String RAIZ_VISITAS = "/visitas";
	public static final String RAIZ_NFE = "/nfe";
	public static final String GLOBO_BROKER = "/globobroker";

	public static final String ESTABELECIMENTOS_TIPOS = "/estabelecimentos/tipos";
	public static final String CONEXAO = "/conexao";
	public static final String VALIDA = "/valida";
	public static final String DBMAKER = "/dbmaker";
	public static final String LOGRADOUROS_TIPOS = "/logradouros/tipos";
	public static final String DOCUMENTOS_TIPOS = "/documentos/tipos";
	public static final String ESTADOS = "/estados";
	public static final String SALVAR = "/salvar";
	public static final String LISTAR = "/listar";
	public static final String BUSCAR = "/buscar";
	public static final String RETIRAR = "/retirar";
	public static final String SINALIZA = "/sinaliza";
	public static final String BOLETO = "/boleto";

}
